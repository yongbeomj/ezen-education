...
public class Day11_4 {
	...
	public static void main(String[] args) throws IOException  {
		...
		// 키보드
			// 키보드 입력 => 입력값(바이트 변환)[a] => 바이트[97] => 문자열 변환[a]
		...
		// p.504
			// 5. indexOf() : 문자열 찾기[인수 : 찾을문자열, 반환 : 인덱스]
			String subject = "자바프로그래밍";
			System.out.println(subject.indexOf("프로그래밍"));
			
			if(subject.indexOf("자바") != -1) { // 인덱스가 -1 : 인덱스가 없다 => 찾을 문자가 존재하지 않음
				System.out.println("자바와 관련된 책입니다");
			} else {
				System.out.println("자바와 관련없는 책입니다");
			}
			// contains() : 문자열 포함여부[인수 : 찾을문자열, 반환 : true/false]
		...
		// p.507
			...
			System.out.println(ssn2.substring(7)); // 시작인덱스부터 끝까지
			
			// 9. split() : 문자열 분리
			String[] ssn3 = ssn2.split("-"); // "-" 기준으로 분해 => 2개의 문자열
			System.out.println(ssn3[0]);
			System.out.println(ssn3[1]);
		
		// p.508
			// 10. toLowerCase() : 소문자 변환, toUpperCase() : 대문자 변환
			String str3 = "Java Programming";
			System.out.println(str3.toLowerCase());
			System.out.println(str3.toUpperCase());
		
		// p.509
			// 11. trim() : 문자열 앞뒤 공백제거 (사이 공백제거 불가)
		...
		// p.510
			...
			String str7 = String.valueOf(true); // 논리형 => 문자열			"true"
			
			System.out.println("문자열 3 : " + str5 + " " + str6 + " " + str7);
