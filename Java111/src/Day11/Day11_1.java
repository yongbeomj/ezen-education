...
public class Day11_1 {
	...
	// p.448 10장 예외 확인문제
		// 1. 4
			// 1. 사용자 조작 혹은 개발자의 잘못된 코드의 오류를 예외라 함
			// 2. 실행예외(실행 도중 예외발생), 일반예외(실행 전 예외발생)
			// 3. try{} catch(예외클래스 변수명){}
			// 4. 개발자가 예외클래스를 설계가능
		// 2. 3
			// 3. finally : 예외유무 상관없이 실행되는 블럭(항상 실행)
		// 3. 4
			// throws : 예외 던지기 => 메소드 안에서 발생된 예외를 호출된 곳으로 전달
			// Exception : 예외클래스 중에 상위클래스
			// throw : 사용자 예외 생성
		// 4. 2
		// 5. 3
			// 3. catch(Exception e) {} catch(ClassNotFoundException e) {}
			// Exception : 예외중에서 상위클래스가 앞에 있을경우 뒤에있는 자식클래스는 의미없음
		// 6. 10 -> 숫자로 변환 할수 없음
			// 순서도 
				// 1. main 메소드 실행 
